...
ALL of the following must be true before stopping:

**Surface coverage (necessary, not sufficient):**
- (1) `--help` rc=0 lists README's documented flags with correct header
- (2) `--version` rc=0 if documented
- (3) ≥3 error paths rc≠0 + non-empty stderr matching grammar
- (4) each subcommand `--help` rc=0, output ≠ main
- (5) no-args matches README behavior

**Behavior probing (REQUIRED — without this, belief stays surface-thin):**
- (6) ≥3 actual usage examples from README executed with real input and observed output
       - e.g. for a JSON tool: `echo '{"a":1}' | bin '.a'` — verify produces `1`
       - e.g. for a text filter: `echo 'foo bar' | bin '-n'` — verify line numbering
       - e.g. for a converter: `bin -i input.fmt -o output.fmt` — verify file emission
- (7) ≥2 distinct INPUT modes tested (stdin / argv / file path), each with a working example
- (8) Categorize the binary's I/O model: text-in/text-out, json-in/json-out, file-in/file-out, etc.

Don't chase byte-exact help unless README ships a fixture (`ngrrram test_full_help_matches_fixture_exactly`). DO chase actual usage — surface probing alone produces a belief that's just a fancy --help rerun.
